...
통계적 추론 (Statistical inference) 또는 통계적 추측은 모집단에 대한 어떤 미지의 양상을 알기 위해 통계학을 이용하여 추측하는 과정을 지칭한다.
...
또한, 이러한 추정 또는 가설 검정을 하기 위해서는 표본의 확률 분포 (`Probability distribution`_)가 어떠한지 파악하는 것이 중요하다.
그 이유는 표본의 확률분포에 따라 검정하는 방법이 달라지기 때문이다 (단, 표본의 수가 많아지면 정규분포를 따름).
물론, 확률 분포의 이해를 위해 확률 (Probability_)에 대한 기본적인 개념은 필요하다.

.. _Probability distribution: https://oi.readthedocs.io/en/latest/statistics/probability_distribution.html
.. _Probability: https://oi.readthedocs.io/en/latest/statistics/probability.html
...
**Reference**
    * `Wikipedia <https://ko.wikipedia.org/wiki/%ED%86%B5%EA%B3%84%EC%A0%81_%EC%B6%94%EB%A1%A0>`_
